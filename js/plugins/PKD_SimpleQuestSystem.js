 ...
 * 翻译：RM兔 qq526915277
 * 
 * 这个插件添加了简单的任务日志（类似于游戏TES Skyrim中的日志）和（在地图上）任务跟踪系统
 *
 * 脚本调用:
 ...
 *      激活\停用地图上的任务跟踪
 *
 * SQSM.isQuestComplete(ID) - 如果带 ID 的任务完成则返回 true(return true if quest with ID is completed)
 * SQSM.isQuestVisible(ID) - 如果将带有 ID 的任务添加到日志中，则返回 true(return true if quest with ID is added to journal)
 * SQSM.isQuestActive(ID) - 如果激活 IDD 任务（已跟踪），则返回 true(return true if quest with IDD is activated (tracked))
 *
 ...
 * @text 光标边距
 * @default {"x":"186","y":"18"}
 * @desc 光标（Quest_Selected.png）边距相对列表项
 * 
 ...
 * @text 活动图标边距
 * @default {"x":"186","y":"18"}
 * @desc 活动任务图标（quest_Active.png）页边距相对列表项
 * 
 ...
 * @desc 设置活动任务帮助图像（ActiveHelp.png）的位置
 * 
 ...
 * @desc 每个活动任务的指针。（仅限PRO版）->指针计数=最大活动任务数
 * 
 ...
 * @desc 通知图像（questJournalUpdated.png）在屏幕上的位置
 * 
 * 
 ...
 * @type text
 * @default 0
 * @desc 编号或脚本（示例：Graphics.width/2）
 *
 * @param y
 * @text Y
 * @type text
 * @default 0
 * @desc 数字或脚本（例如：$gameVariables.value（12）*2）
 */
